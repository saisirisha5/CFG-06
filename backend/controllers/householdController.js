...
// @desc    Get all households and their members for a session
// @route   GET /api/households/session/:sessionId
// @access  Private/Counsellor
const getHouseholdsBySession = async (req, res) => {
    try {
        ...
        if (!session.assignedCounsellor || session.assignedCounsellor.toString() !== counsellorId.toString()) {
            return res.status(403).json({ message: 'You are not authorized to view households for this session.' });
        }

        // 2. Get all households for this session with populated counsellor info
        ...
        const response = {
            ...
            households: households.map(household => ({
                ...
                healthAnalysis: household.healthAnalysis,
                createdAt: household.createdAt,
                updatedAt: household.updatedAt
            })),
            totalHouseholds: households.length,
            totalMembers: households.reduce((total, household) => total + household.members.length, 0)
        };

        res.status(200).json(response);

    } catch (error) {
        res.status(500).json({ message: 'Error fetching households for session', error: error.message });
    }
};
